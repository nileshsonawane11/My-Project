...
if($for == "manage_events"){
    $user_id = $_SESSION['user'];
    $sql = "SELECT * FROM events WHERE owner = '$user_id'";
                    $query = mysqli_query($conn, $sql);
                    if(mysqli_num_rows($query) > 0){
                        echo '<div class="team-list">';
                        ...
                        while($row = mysqli_fetch_assoc($query)){
                            ...
                            $data = <<<TEXT
                                        <div class="team" data-event_id="{$row['id']}" onclick="get_event_info(this)">
                                            <div class="logo"><img src='$src' alt=''></div>
                                            <div class="team-info">
                                                <h4>{$row['e_name']}</h4>
                                                <div class="other-info">
                        ...
                        echo '<h2 class="no-data">No Event Found</h2>';
                    }
}

if($for == "manage_event_tournaments"){
    $user_id = $_SESSION['user'] ?? '';
    ...
    $sql = "
        ...
        JOIN event_tournaments e 
            ON t.tournament_id = e.tournament_id
        WHERE s.sport_name = '$sport'
        ";
                    $query = mysqli_query($conn, $sql);
                    if(mysqli_num_rows($query) > 0){
                        echo '<div class="team-list">';
                        ...
                        while($row = mysqli_fetch_assoc($query)){
                            $canDelete = ($row['created_by'] == $user_id);
                            ...
                            $data = <<<TEXT
                                        <div class="team">
                                            <div class="team_score" data-team_id="{$row['tournament_id']}" onclick="get_team_info(this)">
                                                <div class="logo"><img src="$src" alt=""></div>
                                                    <div class="team-info">
                                                        <h4>{$row['tournament_name']}</h4>
                                                        <div class="other-info">
                                    ...
                                    if ($canDelete) {
                                        $data .= <<<BTN
                                            <div class="delete-btn" onclick="delete_tournament(event, '{$row['tournament_id']}', '{$row['event_id']}')">
