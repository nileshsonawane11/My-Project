...
<dialog id="startMatchDialog">
   ...
   <nav class="nav-bar">
            ...
            <div class="swiper-slide">
                ...
                <div class="comm">
                    <?php
                        ...
                        $total_innings = $score_log['total_innings'] ?? '';
                        for ($inning_key = 1; $inning_key <= $total_innings; $inning_key++) {
                            if (
                                isset($score_log['innings'][$inning_key]['tags']) &&
                                is_array($score_log['innings'][$inning_key]['tags'])
                            ) {
                                foreach ($score_log['innings'][$inning_key]['tags'] as $tag) {
                                    $tag['current_inning'] = $inning_key; // Add the inning key to each raid
                                    $all_tags[] = $tag;
                                }
                            }
                        }
                        ...
                        // $all_tagsd = array_reverse($all_tags);
                    ?>
                    ...
                    <div class="comm-data">
                        ...
                        <?php foreach ($all_tags as $tags): ?>
                            <?php
                                // Extract data from $tags array
                                $point_taken_by = $tags['point_taken_by'] ?? '';
                                $last_score = $tags['last score'] ?? '';
                                $chaser_player = $tags['chaser_player'] ?? '';
                                $team1_points = $tags['team1_points'] ?? '';
                                $team2_points = $tags['team2_points'] ?? '';
                                // $attacking_team = $tags['attacking_team'] ?? '';  // Fix assignment
                                // $defending_team = $tags['defending_team'] ?? '';  // Fix assignment
                                $out_player = $tags['out_player'] ?? '';
                                $type = $tags['type'] ?? ''; // e.g., "tag", "dodge", "self out"
                                $current_inning = $tags['current_inning'] ?? '';
                                $comment = '';

                                // Get Attacking (Chasing) Team Name
                                $curr_attacking_team_name = '';
                                if (!empty($attacking_team)) {
                                    $curr_attacking_team_id = trim($attacking_team);
                                    $curr_attacking_team_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT t_name FROM teams WHERE t_id = '$curr_attacking_team_id' LIMIT 1"));
                                    $curr_attacking_team_name = $curr_attacking_team_row['t_name'] ?? "";
                                }

                                // Get Chaser Player Name
                                $chaser_player_name = '';
                                if (!empty($chaser_player)) {
                                    $chaser_player_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT player_name FROM players WHERE user_id = '$chaser_player' LIMIT 1"));
                                    $chaser_player_name = $chaser_player_row['player_name'] ?? "";
                                }

                                // Get Out Player Name (Runner who is out)
                                $out_player_name = '';
                                if (!empty($out_player)) {
                                    $out_player_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT player_name FROM players WHERE user_id = '$out_player' LIMIT 1"));
                                    $out_player_name = $out_player_row['player_name'] ?? "";
                                }

                                // Get Defending (Running) Team Name
                                $defending_team_name = '';
                                if (!empty($defending_team)) {
                                    $defending_team_id = $defending_team;
                                    $defending_team_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT t_name FROM teams WHERE t_id = '$defending_team_id' LIMIT 1"));
                                    $defending_team_name = $defending_team_row['t_name'] ?? "";
                                }

                                // Get Point Taken By Team Name
                                $point_taken_by_name = '';
                                if (!empty($point_taken_by)) {
                                    $point_taken_by_id = $point_taken_by;
                                    $point_taken_by_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT t_name FROM teams WHERE t_id = '$point_taken_by_id' LIMIT 1"));
                                    $point_taken_by_name = $point_taken_by_row['t_name'] ?? "";
                                }

                                // === Kho-Kho Commentary Generation ===
                                if (!empty($curr_attacking_team_name)) {
                                    if (!empty($chaser_player_name)) {
                                        if (!empty($out_player_name)) {
                                            $comment = "$chaser_player_name from $curr_attacking_team_name tags out $out_player_name of $defending_team_name!";
                                        } elseif ($type === "dodge") {
                                            $comment = "$chaser_player_name from $curr_attacking_team_name attempts a tag but the runner escapes!";
                                        } elseif ($type === "self out") {
                                            $comment = "$out_player_name steps out of bounds! Point awarded to $curr_attacking_team_name.";
                                        } else {
                                            $comment = "$chaser_player_name from $curr_attacking_team_name is chasing... no result yet.";
                                        }
                                    } else {
                                        if (!empty($out_player_name)) {
                                            $comment = "$curr_attacking_team_name gets $out_player_name out.";
                                        } else {
                                            $comment = "$curr_attacking_team_name continues chasing, no runner out yet. Techinical Point to $defending_team_name";
                                        }
                                    }
                                } else {
                                    // Fallback: no attacking team name
                                    if (!empty($out_player_name)) {
                                        $comment = "$out_player_name is out!";
                                    } elseif ($type === "dodge") {
                                        $comment = "Defender dodges successfully.";
                                    } elseif ($type === "self out") {
                                        $comment = "Self-out! Point goes to chasing team.";
                                    } else {
                                        $comment = "Chasing in progress. No outcome yet.";
                                    }
                                }
                        ...
                        <div class="ball-data">
                            <div class="ball-runs">
                                <div class="ball"><?php echo $current_inning; ?></div>
                ...
                <section id="team1">
                    <div class="bowl-data">
                        <?php
                            ...
                            $total_innings = isset($score_log['total_innings']) ? (int)$score_log['total_innings'] : 2;
                            $innings = $score_log['innings'] ?? [];

                            // Calculate full result totals
                            $team1_total = 0;
                            $team2_total = 0;
                            foreach ($innings as $innings_data) {
                                $team1_total += $innings_data['team1_points'] ?? 0;
                                $team2_total += $innings_data['team2_points'] ?? 0;
                            }
                            ...
                            for ($i = $total_innings; $i >= 1; $i--) {
                                $score = $innings[$i]['team1_points'] ?? 0;
                                echo "<td>$score</td>";
                            }
                            echo '</tr>';
                            ...
                            for ($i = $total_innings; $i >= 1; $i--) {
                                $score = $innings[$i]['team2_points'] ?? 0;
                                echo "<td>$score</td>";
                            }
                            echo '</tr>';
    ...
    Object.values(data.innings || {}).forEach(inning => {
        if (inning.tags && Array.isArray(inning.tags)) {
            all_tags = all_tags.concat(inning.tags); // Combine all tags
        }
    });
    
    if (container) container.innerHTML = '';

    if (!data || !Array.isArray(all_tags) || all_tags.length === 0) {
...
console.log(data)
    ...
    const allTags = all_tags.reverse(); // Latest first
    ...
    allTags.forEach((tag, index) => {
        ...
        let current_inning = data.current_inning;
        let attacking_team = data.innings[data.current_inning]?.attack_team;
        let defending_team = data.innings[data.current_inning]?.defence_team;
        let curr_inning = tag.current_inning ?? '';

        // Lookup names
        const attackingTeamName = teamMap[attacking_team] || '';
        const defendingTeamName = teamMap[defending_team] || '';
        const pointTakenByName = teamMap[point_taken_by] || '';

        const chaserPlayerName = playerMap[chaser_player] || '';
        const outPlayerName = playerMap[out_player] || '';

        // === Commentary Logic ===
        let comment = '';

        if (attackingTeamName) {
            if (chaserPlayerName) {
                if (outPlayerName) {
                    comment = `${chaserPlayerName} from ${attackingTeamName} tags out ${outPlayerName} of ${defendingTeamName}!`;
                } else if (type === 'dodge') {
                    comment = `${chaserPlayerName} from ${attackingTeamName} attempts a tag but the runner escapes!`;
                } else if (type === 'self out') {
                    comment = `${outPlayerName} steps out of bounds! Point awarded to ${attackingTeamName}.`;
                } else {
                    comment = `${chaserPlayerName} from ${attackingTeamName} is chasing... no result yet.`;
                }
            } else {
                if (outPlayerName) {
                    comment = `${attackingTeamName} gets ${outPlayerName} out.`;
                } else {
                    comment = `${attackingTeamName} continues chasing, no runner out yet. Technical Point to ${defendingTeamName}.`;
                }
            }
        } else {
            // Fallback
            if (outPlayerName) {
                comment = `${outPlayerName} is out!`;
            } else if (type === 'dodge') {
                comment = `Defender dodges successfully.`;
            } else if (type === 'self out') {
                comment = `Self-out! Point goes to chasing team.`;
        ...
        // Speak latest comment (if speech enabled)
        ...
        ballDiv.innerHTML = `
            <div class="ball-runs">
                <div class="ball">${curr_inning}</div>
                <div class="run">${team1_points} - ${team2_points}</div>
            </div>
...
function update_scoreboard(data) {
    ...
    const team1_name = data.team_map?.[team1_id] || "Team1";
    const team2_name = data.team_map?.[team2_id] || "Team2";

    const total_innings = parseInt(data.total_innings) || 2;
    const innings = Object.values(data.innings || {}); // Convert object to array

    let team1_total = 0;
    let team2_total = 0;

    innings.forEach(inning => {
        team1_total += data.team1_score || 0;
        team2_total += data.team2_score || 0;
    });
    ...
    // Inning headers (2nd, 1st)
    for (let i = total_innings; i >= 1; i--) {
        let suffix = i === 1 ? '1<sup>st</sup>' :
                     i === 2 ? '2<sup>nd</sup>' :
                     `${i}<sup>th</sup>`;
        html += `<th>${suffix}</th>`;
    }
    ...
    html += `<tr>
        <td>${team1_name}</td>
        <td>${winner_team == data.team1 ? 'Win' : '-'}</td>`;

    for (let i = total_innings-1; i >= 0; i--) {
        const score = innings[i]?.team1_points ?? 0;
        html += `<td>${score}</td>`;
    }
    ...
    html += `<tr>
        <td>${team2_name}</td>
        <td>${winner_team == data.team2 ? 'Win' : '-'}</td>`;

    for (let i = total_innings-1; i >= 0; i--) {
        const score = innings[i]?.team2_points ?? 0;
        html += `<td>${score}</td>`;
    }
    ...
    if (matchInfoContainer && data?.innings) {
        const currentInning = data.current_inning;
        const inningData = data.innings[currentInning];

        const attackingTeamId = inningData.attack_team;
        const team1Id = data.team1;
        const team2Id = data.team2;

        const team1_name = data.team_map[team1Id] || "Team 1";
        const team2_name = data.team_map[team2Id] || "Team 2";

        const team1_points = data.team1_score ?? 0;
        const team2_points = data.team2_score ?? 0;

        // Update scores with format "Points (Inning Points)"
        matchInfoContainer.querySelectorAll('.score')[0].innerHTML = `${team1_points}`;
        matchInfoContainer.querySelectorAll('.score')[1].innerHTML = `${team2_points}`;

        // Determine who is attacking
        let team_key = attackingTeamId == team1Id ? 0 : 1;
        let other_team_key = team_key === 0 ? 1 : 0;

        let team_name_key = attackingTeamId == team1Id ? team1_name : team2_name;
        let other_team_name_key = team_key === 0 ? team2_name : team1_name;

        let teams = matchInfoContainer.querySelectorAll('.team');

        // Mark the attacking team with *
